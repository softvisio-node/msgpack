...
function _encode ( bytes, defers, value ) {
    ...
    if ( typeof value === "string" ) {
        ...
        if ( value instanceof Date ) {
            ...
            bytes.push( 0xc7, 12, -1 );

            bytes.push( nano >> 24, nano >> 26, nano >> 8, nano );

            hi = Math.floor( sec / 2 ** 32 );
            lo = sec >>> 0;
            bytes.push( hi >> 24, hi >> 16, hi >> 8, hi, lo >> 24, lo >> 16, lo >> 8, lo );

            // var buf = Buffer.allocUnsafe( 12 );
            // buf.writeUInt32BE( nano );
            // buf.writeBigInt64BE( BigInt( sec ), 4 );

            // defers.push( { "bin": buf, "length": 12, "offset": bytes.length } );
