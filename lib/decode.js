...
class Decoder {
    ...
    #throwRangeError () {
        ...
        throw Error( `MessagePack message in not complete` );
    }
}
...
export function decodeStream ( buffer, offset ) {
    ...
    try {
        ...
        if ( decoder.rangeError || e.code === "ERR_BUFFER_OUT_OF_BOUNDS" || e.code === "ERR_OUT_OF_RANGE" ) return;

        console.log( "---", decoder.rangeError, e.code );
