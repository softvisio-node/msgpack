...
class Decoder {
    ...
    parse () {
        ...
        throw Error( "MessagePack could not parse message" );
    }
    ...
    str ( length ) {
        ...
        // console.log( this.buffer.length, this.offset + lendth )
        // if ( this.buffer.lengtg < this.offset + length ) process.exit();

        if ( this.buffer.length < this.offset + length ) throw Error( `MessagePack message in not complete` );
...
export function decodeStream ( buffer, offset ) {
    ...
    try {
        ...
        if ( e.code === "ERR_BUFFER_OUT_OF_BOUNDS" ) return;
